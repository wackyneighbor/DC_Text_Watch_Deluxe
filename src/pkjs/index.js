...
Pebble.addEventListener('showConfiguration', function(e) {
	...
	var Enlarge = getStorageValue('Enlarge', '1'); // Default enlarge fonts for hi-res watches
	var TimeAlign = getStorageValue('TimeAlign', '5'); // Default middle center time text
	var DateAlign = getStorageValue('DateAlign', '2'); // Default centered date text
	var NoGPS = getStorageValue('NoGPS', '0'); // Default allow regular GPS updates
 	var AnalogElements = getStorageValue('AnalogElements', '1'); // Default display analog elements
	...
	var BackgroundColor2 = getStorageValue('BackgroundColor2', 0x000000); // GColorBlack
 	...
 	var TextDateColor2 = getStorageValue('TextDateColor2', 0xFFFFAA); // GColorPastelYellowhttps://cloudpebble.repebble.com/ide/
	...
	var TextLine2Color3 = getStorageValue('TextLine2Color3', 0xFFFF55); // GColorIcterine
 	...
 	var TimeIndicatorColor3 = getStorageValue('TimeIndicatorColor3', 0x00AAAA); // GColorPersianGreen
    ...
    'Enlarge=' + encodeURIComponent(Enlarge) + '&' +
    'TimeAlign=' + encodeURIComponent(TimeAlign) + '&' +
    'DateAlign=' + encodeURIComponent(DateAlign) + '&' +
    'NoGPS=' + encodeURIComponent(NoGPS) + '&' +
    'AnalogElements=' + encodeURIComponent(AnalogElements) + '&' +
...
Pebble.addEventListener("webviewclosed", function(e) {
	if (e.response !== '') {
		...
		localStorage.setItem('Enlarge', settings.Enlarge);
		localStorage.setItem('TimeAlign', settings.TimeAlign);
		localStorage.setItem('DateAlign', settings.DateAlign);  
		localStorage.setItem('NoGPS', settings.NoGPS);
		localStorage.setItem('AnalogElements', settings.AnalogElements);
    ...
    Pebble.sendAppMessage({"KEY_SINGLE_PREFIX_TYPE" : parseInt(settings.SinglePrefixType, 10),
                       ...
                       "KEY_NOGPS": parseInt(settings.NoGPS, 10),
                       "KEY_ANALOG_ELEMENTS": parseInt(settings.AnalogElements, 10),
                       "KEY_ENLARGE": settings.Enlarge ? 1 : 0,
                       "KEY_TIME_ALIGN": parseInt(settings.TimeAlign, 10),
                       "KEY_DATE_ALIGN": parseInt(settings.DateAlign, 10),
                       "KEY_START_DEMO": parseInt(settings.start_demo_now, 10),
                      });  
	}
  });